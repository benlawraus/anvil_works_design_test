...
rsync -a --exclude='_anvil_designer.py' --exclude='__pycache__' --include='*.py' --include='*/' --exclude='*'  ./client_code/ "$anvil_app"/client_code
rsync -a --exclude='__pycache__' --include='*.py' --include='*/' --exclude='*'  ./server_code/ "$anvil_app"/server_code
if git -C $anvil_app commit -am "Edited on laptop"; then
  ...
  if git -C $anvil_app diff --exit-code; then
    ...
    git add -C $anvil_app . 1>/dev/null 2>/dev/null
